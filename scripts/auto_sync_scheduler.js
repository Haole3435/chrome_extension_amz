...
function scheduleAutoSync() {
  ...
  // Đặt thời gian đồng bộ là 9h sáng
  ...
  scheduledSyncTask = setTimeout(() => {
    ...
    chrome.tabs.query({}, (tabs) => {
      ...
      if (amazonTab) {
        ...
        // Bắt đầu đồng bộ sau 2 giây
        setTimeout(() => {
          startAutoSync(amazonTab.id);
        }, 2000);
      } else {
  ...
  chrome.tabs.create({ url: url, active: true }, (tab) => {
    ...
    function handleTabUpdate(tabId, changeInfo) {
      if (tabId === tab.id && changeInfo.status === 'complete') {
        ...
        setTimeout(() => {
          console.log('[AUTO SYNC] Bắt đầu đồng bộ sau khi trang đã ổn định');
          startAutoSync(tab.id);
